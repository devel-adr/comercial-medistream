...
import { Search, Eye, Link, ArrowUp, ArrowDown, BarChart3, Trash2, Star, StarOff } from 'lucide-react';
import { MedicationDetailModal } from './MedicationDetailModal';
import { toast } from "@/hooks/use-toast";
import { supabase } from '@/integrations/supabase/client';

interface EnhancedMedicationsTableProps {
  medications: any[];
  loading: boolean;
  searchTerm: string;
...
export const EnhancedMedicationsTable: React.FC<EnhancedMedicationsTableProps> = ({
  medications = [],
  loading,
  searchTerm,
  onSearchChange,
  ...
  const handleToggleFavorite = (idNum: string) => {
    const newFavorites = new Set(favorites);
    if (newFavorites.has(idNum)) {
      newFavorites.delete(idNum);
      toast({
        title: "Favorito eliminado",
        description: "Medicamento eliminado de favoritos",
      });
    } else {
      newFavorites.add(idNum);
      toast({
        title: "Favorito añadido",
        description: "Medicamento añadido a favoritos",
      });
    }
  ...
  const handleDelete = async (medication: any) => {
    const idNum = medication.ID_NUM?.toString();
    if (!idNum) return;

    setDeletingIds(prev => new Set([...prev, idNum]));
    ...
    try {
      ...
      toast({
        ...
        description: `${medication.nombre_del_farmaco || 'Medicamento'} ha sido eliminado correctamente.`,
      });
                  ...
                  <Table className="w-full">
                    <TableHeader className="sticky top-0 z-10 bg-gray-50 dark:bg-gray-800">
                      <TableRow className="border-b">
                        ...
                        <TableHead className="w-[50px] text-center font-semibold">Fav</TableHead>
                        <TableHead className="w-[50px] text-center font-semibold">ID</TableHead>
                        <TableHead 
                    ...
                    <TableBody>
                      {paginatedData.map((medication, index) => {
                        const idNum = medication.ID_NUM?.toString();
                        const isSelected = selectedMedicationIds.has(idNum);
                        const isFavorite = favorites.has(idNum);
                        const isDeleting = deletingIds.has(idNum);
                        ...
                        return (
                          <TableRow 
                            ...
                            } ${isFavorite ? 'bg-yellow-50 dark:bg-yellow-900/20' : ''} ${
                          ...
                          >
                            <TableCell className="text-center">
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => 
                                  handleSelectMedication(idNum, checked === true)
                                }
                                disabled={isDeleting}
                              />
                            </TableCell>
                            <TableCell className="text-center">
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleToggleFavorite(idNum)}
                                disabled={isDeleting}
                            ...
                            <TableCell>
                              <div className="w-[130px] truncate" title={medication.sub_area_de_tratamiento}>
                                {truncateText(medication.sub_area_de_tratamiento)}
                              </div>
                            </TableCell>
                            <TableCell>
